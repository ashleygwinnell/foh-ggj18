...
void MyParticle::reset() {   
	...
	m_lifeRotationStart = 0.0f;
	m_lifeRotationEnd = 0.0f;
...
void MyParticle::render(GameContainer* container, Renderer* r) {
	...
	if (m_lifetime) {
		...
		rotationLife = Easing::easebetween(Easing::QUADRATIC_OUT, m_lifeTimer, m_lifeRotationStart, m_lifeRotationEnd, m_lifeDuration);
	}

	float rotation = m_rotationOffset + rotationLife;
	...
	if (m_type == TYPE_TEXT) {
		...
		r->drawString(m_text, m_bounds->getCenterX(), m_bounds->getCenterY(), Renderer::ALIGN_CENTER, Renderer::ALIGN_CENTER, rotation, scale);
	}
